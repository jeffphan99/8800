...
namespace Unity.Template.Multiplayer.NGO.Runtime
{
    ...
    public class ConfigurationManager
    {
        ...
        public ConfigurationManager(MonoBehaviour routineRunner, string configFilePath, System.Action<ConfigurationManager> onFinished = null)
        {
            string fullPath;
#if UNITY_EDITOR
            // In the editor, we load the config file from the project root.
            fullPath = configFilePath;
            Debug.Log($"[ConfigurationManager] EDITOR - Loading config from: {fullPath}");
#else
            // In a standalone build, we load the config file from the StreamingAssets folder.
            fullPath = Path.Combine(Application.streamingAssetsPath, configFilePath);
            Debug.Log($"[ConfigurationManager] STANDALONE BUILD - Loading config from: {fullPath}");
#endif

#if UNITY_ANDROID && !UNITY_EDITOR
            Debug.Log($"Loading config on Android platforms, from {fullPath}");
            routineRunner.StartCoroutine(LoadConfigurationFromFileOnNonDesktop(fullPath, onFinished));
            return;
#endif

            LoadConfigurationFromFile(fullPath, false, false);
        ...
        {
            Debug.Log($"[ConfigurationManager] Attempting to load config from: {configFilePath}");
            m_ConfigFilePath = configFilePath;
            string templatePath = Path.Combine("DefaultConfigurations", configFilePath.Split('.')[0]);
            if (!File.Exists(configFilePath))
            {
                Debug.LogError($"[ConfigurationManager] File does not exist: {configFilePath}");
            ...
            Debug.Log($"[ConfigurationManager] Config loaded successfully! Content: {m_Config.ToString()}");
            if (!updateIfOutdated)
            {
                return;
            }
